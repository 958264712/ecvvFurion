	...
	<div class="collectionOrderInfo-container">
		...
		<el-card class="full-table" shadow="hover" style="margin-top: 8px">
			<el-form-item class="settingf">
				<el-button type="primary" icon="ele-Plus" @click="examines()" style="margin-right: 20px"> 新增集货单商品基础信息
				</el-button>
			...
			<el-table :data="tableData" size="large" style="width: 100%" v-loading="loading" tooltip-effect="light"
				...
				<template v-for="el in columns" :key="el.dataIndex">
					<el-table-column v-if="el.checked" :prop="el.dataIndex" :label="el.title" align="center"
						:width="el.width" sortable="custom">
						<template #default="scope" v-if="el.title == '产品图'">
							<img :src="'https://raw.githubusercontent.com/okbuynow/OKPIC/main/' + scope.row.internalUniqueID + '.jpg'"
								alt="" style="width: 80%" />
						</template>
					</el-table-column>
				</template>
		...
		<el-dialog v-model="dialogFormVisible" title="新增集货单商品基础信息">
			<el-form label-width="130px">
				<el-form-item v-for="el in columns" :key="el.dataIndex" :label="el.title"
					style="width: 80%; margin: 30px auto">
					<el-input v-model="el.values" :placeholder="`请输入${el.title}`" style="width: 100%" />
				</el-form-item>
			</el-form>
			<template #footer>
...
let columns = ref<any>([
	...
	},
	{
		...
		//       var imgsrc = 'https://raw.githubusercontent.com/okbuynow/OKPIC/main/' + row.internalUniqueID + '.jpg'
	...
	//   width: 100,
	//   sorter: 'custom',
	//   dataIndex: 'dubaiReceiptQuantity'
	// },
	{
		title: '迪拜仓库收货数量',
		align: 'center',
