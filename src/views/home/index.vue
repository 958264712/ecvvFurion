			...
			<el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" v-for="(v, k) in state.homeOne" :key="k" :class="{ 'home-media home-media-lg': k > 1, 'home-media-sm': k === 1 }">
				<div class="home-card-item flex">
					<div class="flex-margin flex w100" :class="` home-one-animation${k}`">
						<div class="flex-auto">
							<div class="mt10" style="margin-bottom: 10px;font-weight: bold;font-size: large;">{{ v.name }}</div>
							<span class="font30">{{ v.num1 }}</span>
							<div class="mt10">{{ v.num3 }}<span :class="v.runState ? 'green' : 'red'">{{ v.time }}</span></div>
						</div>
						<template v-if="v.runState">
							<Loading style="width: 40px;color:green"/>
						</template>
						<template v-else>
