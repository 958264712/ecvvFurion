<script lang="ts" setup name="warehouse_name">
import { ref, onMounted } from 'vue';
import { getSitesWareHouse, wareHouseManagementPage, wareHouseManagementDelete } from '/@/api/modular/main/basicInfo.ts';
...
const siteList = ref<any>([{ label: '全部', value: null }]);
const siteItemList = ref<any>([]);
const queryParams = ref<any>({ state: '全部', relatedSite: '全部' });
...
const handleQuery = async () => {
	...
	var res = await wareHouseManagementPage(Object.assign(tableParams.value, queryParams.value));
...
const deleteItem = (row: any) => {
	ElMessageBox.confirm(`确定要删除吗?`, '提示', {
		...
		.then(async () => {
			await wareHouseManagementDelete(row);
			handleQuery();
		...
		<el-card shadow="hover" :body-style="{ paddingBottom: '0' }">
			<el-form :model="queryParams" ref="queryForm" :inline="true">
				<el-form-item label="仓库名称">
					<el-input v-model="queryParams.warehouse" type="text" placeholder="请输入" />
				</el-form-item>
		...
		<el-card class="full-table" shadow="hover">
			<div style="display: flex; justify-content: space-between">
				<div style="margin-bottom: 20px; display: flex; justify-content: space-between">
					<el-button type="primary" @click="openAddManagement" v-auth="'warehouseName:add'">新增</el-button>
				</div>
				<tabDragColum :data="TableData" :name="`warehouse_name`" :area="area" @handleData="handleData" />
			</div>
			...
			<el-table :data="tableData" style="height: 100%" v-loading="loading" tooltip-effect="light" row-key="id" border="">
				...
				<el-table-column label="操作" align="center" fixed="right" width="180" v-if="auth('warehouseName:update') || auth('warehouseName:delete')">
					<template #default="scope">
						<el-button size="small" text type="primary" @click="openEdit(scope.row)" v-auth="'warehouseName:update'"> 编辑 </el-button>
						<el-button size="small" text type="primary" @click="deleteItem(scope.row)" v-auth="'warehouseName:delete'"> 删除 </el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination
