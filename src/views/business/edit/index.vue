	...
	<div class="sys-menu-container">
			...
			<el-row style="display: flex; justify-content: space-around">
				<el-form-item label="单据编号" required="true">
					<el-input :formatter="formatInput" :class="!collectionOrderInfo.documentNo && buttonmask ? 'avt' : ''"
						v-model="collectionOrderInfo.documentNo" clearable="" :disabled="compile == 0 || true"
						placeholder="请输入单据编号" />
					...
					<el-row>
						<el-col :span="8">
							<el-form-item label="收货人" prop="consigneeName" style="width: 90%">
								<el-input :disabled="compile == 0" v-model="collectionOrderInfo.consigneeName" clearable=""
									placeholder="请输入收货人" />
							</el-form-item>
							...
							<el-form-item label="备注" style="width: 90%">
								<el-input :disabled="compile == 0" v-model="collectionOrderInfo.notes" type="textarea"
									placeholder="请输入备注" />
							</el-form-item>
						</el-col>
						<el-col :span="8" style="">
							<el-form-item label="物流报价" style="width: 90%" required="true">
								<el-input :type="'number'" :formatter="formatInput"
									...
									:disabled="compile == 0" v-model="collectionOrderInfo.logisticsPrice" clearable=""
									placeholder="请输入" />
							...
							<el-form-item label="是否报关"> <el-switch :disabled="compile == 0"
							...
							<el-form-item label="运输方式" style="width: 90%" required="true">
								<el-select :class="!collectionOrderInfo.shippingMethod && buttonmask ? 'avt' : ''"
									v-model="collectionOrderInfo.shippingMethod" :disabled="compile == 0" filterable
									clearable>
							...
							<el-form-item label="收件人" style="width: 90%">
								<el-input :disabled="compile == 0" v-model="collectionOrderInfo.recipientName" clearable=""
									placeholder="请输入收件人" />
							</el-form-item>
							<el-form-item label="出发日期" style="width: 90%">
								<el-date-picker :disabled="compile == 0" v-model="collectionOrderInfo.departureDate"
									type="date" placeholder="请选择日期" />
							</el-form-item>
							<el-form-item label="送仓日期" style="width: 90%">
								<el-date-picker :disabled="compile == 0" v-model="collectionOrderInfo.deliveryDate"
									type="date" placeholder="请选择日期" />
							</el-form-item>
							<el-form-item label="创建日期" style="width: 90%">
								<el-date-picker :disabled="compile == 0" v-model="collectionOrderInfo.orderCreatedDate"
									type="date" placeholder="请选择日期" />
							</el-form-item>
							<el-form-item label="附件" style="width: 90%">
								<el-upload v-model:file-list="fileList"
									...
									name="file" :before-remove="handleRemovefun" :disabled="compile == 0">
									<el-button :disabled="compile == 0" type="primary" :loading="loading1" icon="ele-Plus"
						...
						<el-col :span="8">
							<el-form-item label="汇率" style="width: 90%" required="true">
								<el-input :type="'number'" :formatter="formatInput"
									...
									:disabled="compile == 0" v-model="collectionOrderInfo.exchangeRate" clearable=""
									placeholder="请输入" />
							...
							<el-form-item label="报关费" style="width: 90%" required="true">
								<el-input :type="'number'" :formatter="formatInput"
									...
									:disabled="compile == 0" v-model="collectionOrderInfo.totalCustomsDeclarationFee"
							...
							<el-form-item label="货代入仓号" style="width: 90%">
								<el-input :disabled="compile == 0" v-model="collectionOrderInfo.inWareHouseNo" clearable=""
									placeholder="请输入货代入仓号" />
							</el-form-item>
							<el-form-item label="收件联系电话" style="width: 90%">
								<el-input :type="'number'" :disabled="compile == 0"
									v-model="collectionOrderInfo.recipientPhone" clearable="" placeholder="请输入收件联系电话" />
							</el-form-item>
							<el-form-item label="预计到港日期" style="width: 90%">
								<el-date-picker :disabled="compile == 0" v-model="collectionOrderInfo.estimatedArrivalDate"
									type="date" placeholder="请选择日期" />
							</el-form-item>
							<el-form-item label="实际送仓日期" style="width: 90%">
								<el-date-picker :disabled="compile == 0" v-model="collectionOrderInfo.actualDeliveryDate"
									type="date" placeholder="请选择日期" />
							</el-form-item>
							<el-form-item label="创建人" style="width: 90%">
								<el-input :disabled="compile == 0" v-model="collectionOrderInfo.orderCreatedUser"
									clearable="" placeholder="请输入" />
							</el-form-item>
						</el-col>
						<div style="margin: 0 auto; position: relative">
							<el-button :disabled="!isCommit || compile == 0" type="primary" @click="commit()">提交</el-button>
		...
		<el-card class="full-table" shadow="hover" style="margin-top: 8px" v-show="id">
			...
			<el-table class="table-container" v-loading="state.loading" :data="collectionGoodsInfolist"
				style="width: 100%; margin-top: 10px" ref="myTable" @selection-change="handleSelectionChange">
				...
				<el-table-column fixed prop="internalUniqueID" label="内部唯一识别码" :width="widths">
					<template #default="scope">
						...
						<el-autocomplete :formatter="formatInput" v-if="scope.row.rowCompile || allCompiles"
							...
							@select="handleSelect(scope)" @focus="focuefun(scope)" @blur="blurfun(scope)"></el-autocomplete>
					</template>
				</el-table-column>
				...
				<el-table-column prop="supplier" label="供应商" width="180">
					<template #default="scope">
						<el-input v-if="scope.row.rowCompile || allCompiles" @focus="focuefun(scope)"
							v-model="scope.row.supplier"></el-input>
					</template>
				</el-table-column>
...
import { onMounted, reactive, ref, toRefs, computed, watch, watchEffect, onBeforeUnmount, onActivated } from 'vue';
...
let weightKG = (row: any) => {
	...
	if (collectionOrderInfo.neetCustoms) {
		if (collectionOrderInfo.totalCustomsDeclarationFee && sumResult.value.customsClearanceProducts) {
			row.customsDeclarationFee = roundToThreeDecimalPlaces(collectionOrderInfo.totalCustomsDeclarationFee / sumResult.value.customsClearanceProducts);
			console.log(row.customsDeclarationFee);
		}
	} else {
		row.customsDeclarationFee = 0;
	}
