	...
	<div class="collectionOrderInfo-container">
		...
		<el-card class="full-table" shadow="hover" style="margin-top: 8px">
			...
			<el-table :data="tableData" size="large" style="width: 100%" v-loading="loading" tooltip-effect="light"
				row-key="id" @selection-change="handleSelectionChange" @sort-change="sortfun">
				<el-table-column type="selection" width="55" />
				<template v-for="(item, index) in CollectionOrderTable" :key="index">
					<el-table-column v-if="item.checked && index === 0" :prop="item.dataIndex" :label="item.title"
						:align="item.align" sortable="custom" :width="item.width" fixed="left" show-overflow-tooltip="">
					...
					<el-table-column
						v-if="item.checked && index != 0 && (index !== CollectionOrderTable.length - 1 || item.title !== '操作')"
						:prop="item.dataIndex" :label="item.title" :align="item.align" sortable="custom" :width="item.width"
						show-overflow-tooltip="" />
					<el-table-column v-if="item.checked && index === CollectionOrderTable.length - 1 && item.title === '操作'"
...
function CollectionOrderReset() {
	...
	console.log(OrderColumns.value);
	service({
		url: `/api/tableShowColumnConfig/save`,
		method: 'post',
		data: {
