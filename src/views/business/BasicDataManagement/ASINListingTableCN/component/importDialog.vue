		...
		<el-dialog v-model="isShowDialog" title="导入" :width="800" @close="closeDialog(ruleFormRef)" draggable="" center>
				...
				<div class="moban">
					<div style="display: flex; align-items: center">
						<svg t="1709867915105" class="icon" viewBox="0 0 1024 1024" version="1.1"
							xmlns="http://www.w3.org/2000/svg" p-id="2468" width="64" height="64">
							<path
			...
			<div class="import" v-if="isShowImport">
				...
				<el-upload :on-change="Imports" :multiple="false" :show-file-list="false" :auto-upload="false"
					...
					<div class="moban1">
						<svg t="1709867915105" class="icon" viewBox="0 0 1024 1024" version="1.1"
							xmlns="http://www.w3.org/2000/svg" p-id="2468" width="128" height="128">
							<path
								d="M854.6 288.6L639.4 73.4c-6-6-14.1-9.4-22.6-9.4H192c-17.7 0-32 14.3-32 32v832c0 17.7 14.3 32 32 32h640c17.7 0 32-14.3 32-32V311.3c0-8.5-3.4-16.7-9.4-22.7zM790.2 326H602V137.8L790.2 326z m1.8 562H232V136h302v216c0 23.2 18.8 42 42 42h216v494z"
								p-id="2469" fill="#707070"></path>
							<path
			...
			<div v-if="!isShowImport">
				...
				<div v-else>
					...
					<template v-else>
						导入失败，总<span class="success">{{ result?.total }}</span>条，成功导入<span class="success">0</span>
						条数据，失败<span class="fail">{{ result?.fail }}</span>条，可下载错误报告，修改后再次上传
						<el-table :data="result?.items" size="lagre" style="width: 100%; margin: 20px 0"
							v-loading="loading1" tooltip-effect="light" row-key="id" border="">
							<el-table-column prop="rowNO" label="行数" width="80" sortable align="center"
								show-overflow-tooltip="" />
							<el-table-column prop="asin" label="ASIN" min-width="150" sortable align="center"
								show-overflow-tooltip="" />
							<el-table-column prop="erpSku" label="ERP-SKU" min-width="170" sortable align="center"
								show-overflow-tooltip="" />
							<el-table-column prop="addASINAccount" label="上架平台" min-width="150" sortable align="center"
								show-overflow-tooltip="" />
							<el-table-column prop="errorInfo" label="失败原因" min-width="150" sortable align="center"
								show-overflow-tooltip="">
						...
						<el-button type="primary" :loading="Exportloading" @click="exportExcel"
							style="margin: 10px auto 0">下载报告</el-button>
					</template>
				</div>
			</div>
		</el-dialog>
