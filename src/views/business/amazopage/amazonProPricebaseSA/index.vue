<script lang="ts" setup name="amazonProPricebaseSA">
import { ref } from 'vue';
import { ElMessageBox, ElMessage } from 'element-plus';
import { auth } from '/@/utils/authFunction';
// import editDialog from './component/editDialog.vue'
import {
	pageAmazonProPricebaseSA,
	deleteAmazonProPricebaseSA,
	updateAmazonProPricebaseSA,
	addAmazonProPricebaseSA,
	AmazondataimportSA,
	AmazondataExportSA,
...
const handleQuery = async () => {
	loading.value = true;
	var res = await pageAmazonProPricebaseSA(Object.assign(queryParams.value, tableParams.value));
...
const delAmazonProductPricebase = (row: any) => {
	ElMessageBox.confirm(`确定要删除吗?`, '提示', {
		...
		.then(async () => {
			await deleteAmazonProPricebaseSA(row);
			handleQuery();
...
const AddAmazonProPricebase = async () => {
	await addAmazonProPricebaseSA(Object.assign(addForm.value)).then((res) => {
...
const monitorChange = (value: string, record: any) => {
	...
	updateAmazonProPricebaseSA(record).then((res) => {
...
const AmazonProductPricebaseAdjustPrices = (record: any) => {
	AmazonProductPricebaseAdjustPriceSA(record).then((res) => {
		if (res.data.type === 'success') {
...
const Amazonimport = useDebounce((file: any) => {
	...
	AmazondataimportSA(formData).then((res) => {
...
const AmazonBatchDelete = () => {
	ElMessageBox.confirm(`您确定要删除这${selectedRowKeys.value.length}条数据吗？`, 'Danger', {
		confirmButtonText: 'OK',
		cancelButtonText: 'Cancel',
		type: 'warning',
	}).then(() => {
		AmazonProductBatchDeleteSA(selectedRowKeys.value).then((res) => {
			if (res.data.type === 'success') {
				ElMessage.success('删除成功');
			...
			<el-table :data="tableData" size="large" style="width: 100%" v-loading="loading" tooltip-effect="light" row-key="id" border="" @selection-change="(selection: any) => selectChange(selection)">
				...
				<el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="">
					<template #default="scope">
						...
						<el-popconfirm title="确定调价?" @confirm="AmazonProductPricebaseAdjustPriceSA(scope.row)">
							<template #reference>
