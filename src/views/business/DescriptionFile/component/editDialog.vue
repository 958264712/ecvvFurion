		...
		<el-dialog v-model="isShowDialog" :title="props.title" :width="800" @close="closeDialog(ruleFormRef)" draggable="">
			<el-form :model="ruleForm" ref="ruleFormRef" size="default" label-width="120px" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="表格模板说明：" prop="fileAddress">
...
import { uploadDescriptionFile } from '/@/api/main/DescriptionFile';
import { addTableImport, updateTableImport } from '/@/api/main/DescriptionFile';
//父级传递来的参数
...
const uploadtableDescriptionAddressHandle = async (options: any) => {
	console.log(1);
	console.log(options);
	const formData = new window.FormData();
	formData.append('file', options.file);

	const res = await uploadDescriptionFile(formData);
	console.log(res);
	ruleForm.value.fileAddress = res.data.result?.url;
};
// 页面加载时
onMounted(async () => { });
