<script lang="ts" setup name="saudiPoData">
import { ref, h } from 'vue';
import { ElMessageBox, ElMessage } from 'element-plus';
import { auth } from '/@/utils/authFunction';
//import { formatDate } from '/@/utils/formatTime';
import { saudiPagePoData, deletePoData, updateOrderDate, ImportPO, bulkImportOrderDate, multipleExportByTemplate, downLoadPOZip } from '/@/api/modular/main/aSINBasicData.ts';
...
const handleQuery = async () => {
	loading.value = true;
	var res = await saudiPagePoData(Object.assign(queryParams.value, tableParams.value));
...
const delPoData = (row: any) => {
	ElMessageBox.confirm(`Are you sure you want to delete it?`, 'Prompt Message', {
		...
		.catch(() => {
			ElMessage.success('Failed Delete');
		});
};
		...
		.catch((res) => {
			...
			ElMessageBox({
				message: h(
					...
					`${result} in the data has already been imported. If you needs to be imported again, the original ${result} will be overwritten: it is a duplicate PO number`
				),
...
const multipleExport = () => {
	...
	multipleExportByTemplate({ poList: data, Site: 'SA' }).then((res: any) => {
...
<template>
	<div class="poData-container">
		...
		<el-card class="full-table" shadow="hover" style="margin-top: 8px">
          ...
          > -->
				<el-link icon="ele-Download" :underline="false" href="https://sa1api.ecvv.com/ExcelTemplate/POtemplate.xlsx"> Download the PO import template</el-link>
				<el-link icon="ele-Download" :underline="false" href="https://sa1api.ecvv.com/ExcelTemplate/OrderDate.xlsx"> Download the OrderDate import template</el-link>
			</div>
			<el-table :data="tableData" style="width: 100%" v-loading="loading" tooltip-effect="light" row-key="id" size="lagre" border="" @selection-change="(selection: any) => selectChange(selection)">
				...
				<el-table-column prop="isGenerate" label="IsGenerate" align="center" show-overflow-tooltip="">
			...
			<el-dialog v-model="visible1" title="Edit OrderDate">
