				...
				<el-col :span="12">
					<el-form-item label="原币种" :rules="[{ required: true, message: `人员不能为空`, trigger: 'blur' }]">
						<el-select v-model="ruleForm.originalCurrency" clearable="" class="select" placeholder="请选择">
							<el-option v-for="(item, index) in props.moneyList" :key="index" :value="item.value"
								:label="item.label" />
						</el-select>
					</el-form-item>
					<el-form-item label="汇率" :rules="[{ required: true, message: `汇率不能为空`, trigger: 'blur' }, {}]">
						<el-input v-model="ruleForm.exchangeRate" type="number" style="height:30px;width:250px"
							placeholder="请输入汇率" />
					</el-form-item>
				</el-col>
				<el-col :span="12">
					<el-form-item label="兑换币种" :rules="[{ required: true, message: `年份不能为空`, trigger: 'blur' }]">
						<el-select v-model="ruleForm.exchangeCurrency" clearable="" class="select" placeholder="请选择">
							<el-option v-for="(item, index) in props.moneyList1" :key="index" :value="item.value"
								:label="item.label" />
						</el-select>
					</el-form-item>
					<el-form-item label="生效时间" :rules="[{ required: true, message: `年度目标值不能为空`, trigger: 'blur' }]">
						<el-date-picker :clearable="false" v-model="ruleForm.takeEffectTime" type="month"
							style="height:30px;width:250px" placeholder="请选择月份" />
					</el-form-item>
				</el-col>
			</el-row>
